        ...
        <ul class="relative w-full mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <?php foreach ($recipes as $recipe) : ?>
                <li class="relative h-[200px] lg:h-[300px] w-full flex justify-between">
                    <a href="<?= str_replace('{id}', $recipe->id, $routes->get('recipe')->getPath()); ?>" class="relative w-full h-full p-5 flex flex-col justify-end items-start rounded-xl bg-[url(<?= $recipe->thumbnail_url; ?>)] bg-cover bg-center shadow-[inset_0_-120px_60px_-60px_rgba(34,34,34,0.9)]">
